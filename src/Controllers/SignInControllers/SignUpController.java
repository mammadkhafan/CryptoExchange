...
public class SignUpController extends SignInMethods implements Initializable{
    ...
    public void changeCaptchaCodeImage() {
        int lastIndexOfCaptchase = captchaImages.length;
        randomIndex = (int)(Math.random() * (lastIndexOfCaptchase));        
    ...
    private void checkCaptchaCode(ActionEvent event) {
        ...
        if(!captchaCode[randomIndex].equals(captchaCodePasswordField.getText())){
            captchaMessage.setText("Invalid input");   
            captchaMessage.setTextFill(red);
        }else{
            captchaMessage.setTextFill(green);
            captchaMessage.setText("Valid input");   
        }
    }
